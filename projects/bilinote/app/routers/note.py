...
def save_note_to_file(task_id: str, note):
    ...
    data = asdict(note) if hasattr(note, '__dataclass_fields__') else note
...
def get_task_status(task_id: str):
    ...
    def slim_result(content):
        """精简返回结果，去掉 transcript.segments 防止前端 localStorage 超限"""
        if isinstance(content, dict) and "transcript" in content:
            t = content["transcript"]
            if isinstance(t, dict) and "segments" in t:
                content["transcript"] = {
    ...
    if os.path.exists(status_path):
        ...
        if status == TaskStatus.SUCCESS.value:
            ...
            if os.path.exists(result_path):
                ...
                return R.success({
                    "status": status,
                    "result": slim_result(result_content),
                    "message": message,
                    "task_id": task_id
                })
            else:
