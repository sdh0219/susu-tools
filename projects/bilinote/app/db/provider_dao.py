...
def _safe_provider_summary(row) -> tuple:
    """日志用：隐藏 api_key，避免密钥写入日志文件。"""
    if not row:
        return row
    row = list(row)
    if len(row) > 4:
        row[4] = _mask_key(row[4])
    return tuple(row)
...
def get_enabled_providers():
    ...
    try:
        ...
        if rows is None:
            ...
            return None
        logger.info(f"Providers found: {len(rows)} enabled")
        return rows
    except Exception as e:
...
def get_provider_by_name(name: str):
    ...
    try:
        ...
        if row is None:
            ...
            return None
        logger.info(f"Provider found: {_safe_provider_summary(row)}")
        return row
    except Exception as e:
...
def get_all_providers():
    ...
    try:
        ...
        if rows is None:
            logger.info("No providers found")
            return None
        logger.info(f"Providers found: {len(rows)} total")
        return rows
    except Exception as e:
...
def update_provider(id: str, **kwargs):
    ...
    for key, value in kwargs.items():
        fields.append(f"{key} = ?")
        values.append(value)
        log_fields.append(f"{key}={'***' if key == 'api_key' else value}")
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Provider updated successfully. id: {id}, updated_fields: {log_fields}")
    except Exception as e:
